...
async def process_intent_result(conn, intent, original_text):
    ...
    # 处理定时泡燕麦意图
    if "泡燕麦" in intent or "冲燕麦" in intent:
        logger.bind(tag=TAG).info(f"识别到定时泡燕麦意图: {intent}")
        # 提取时间描述
        time_str = extract_text_in_brackets(intent)
        if not time_str:
            # 如果没有在中括号中提取到时间，尝试从原始文本中提取
            time_str = original_text
            
        function_id = str(uuid.uuid4().hex)
        function_name = "oatmeal_timer"
        function_arguments = '{ "time_str": "' + time_str + '" }'
